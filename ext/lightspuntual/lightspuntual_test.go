...
func TestLight_UnmarshalJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{"default", new(Light), args{[]byte("{}")}, &Light{
			Color: &[3]float64{1, 1, 1}, Intensity: gltf.Float(1), Range: gltf.Float(float64(math.Inf(0))),
		}, false},
...
func TestUnmarshal(t *testing.T) {
	...
	tests := []struct {
		...
		{"lights", args{[]byte(`{"lights": [
			...
			{Color: &[3]float64{1, 0.9, 0.7}, Name: "Directional", Intensity: gltf.Float(3.0), Type: "directional", Range: gltf.Float(float64(math.Inf(0)))},
			{Color: &[3]float64{1, 0, 0}, Name: "Point", Intensity: gltf.Float(20.0), Type: "point", Range: gltf.Float(float64(math.Inf(0)))},
		}, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
